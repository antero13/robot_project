...
class DeterministicPickupControllerTest(unittest.TestCase):
    ...
    def test_avoidance_runs_turn_then_forward_without_a_visible_target(self):
        ...
        escaping = self.controller.command(now_s=2.5)
        finished = self.controller.command(now_s=3.3)
        ...
        self.assertEqual(escaping.state, self.controller.AVOID_ESCAPE)
        self.assertGreater(escaping.linear_x, 0.0)
