...
class AvoidanceRoiTest(unittest.TestCase):
    def test_accepts_points_in_calibrated_robot_corridor(self):
        self.assertTrue(
            point_is_inside_trapezoid_roi(0.0, 0.30, **CALIBRATED_ROI)
        )
        self.assertTrue(
            point_is_inside_trapezoid_roi(0.0, 0.50, **CALIBRATED_ROI)
        )

    def test_rejects_points_beside_or_beyond_corridor(self):
        self.assertFalse(
            point_is_inside_trapezoid_roi(-0.70, 0.50, **CALIBRATED_ROI)
        )
        self.assertFalse(
            point_is_inside_trapezoid_roi(0.50, 0.50, **CALIBRATED_ROI)
        )
        self.assertFalse(
            point_is_inside_trapezoid_roi(0.0, 0.20, **CALIBRATED_ROI)
        )
        self.assertFalse(
            point_is_inside_trapezoid_roi(0.0, 0.80, **CALIBRATED_ROI)
        )
    ...
    def test_includes_interpolated_side_boundaries(self):
        ...
        self.assertTrue(
            point_is_inside_trapezoid_roi(left_x, y, **CALIBRATED_ROI)
        )
        self.assertFalse(
