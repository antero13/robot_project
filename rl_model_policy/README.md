...
정방향 1→2→3→4 이동은 서쪽 벽 `x=-2.0`, 보관소 이후 역방향
4→3→2→1 이동은 동쪽 벽 `x=2.0`을 사용한다. 여기서 left/right wall은
경기장의 서쪽/동쪽 벽 좌표이며, 차체에 달린 두 센서의 좌우를 뜻하지 않는다.
...
1. **레인 탐색**
   ...
   - 다음 레인 이동은 odometry waypoint로 먼저 수행하고, 10 cm 허용
     오차에 들어온 뒤 ToF로 남은 x 오차를 3 cm 이내로 맞춘다.
...
6. **주도로 복귀와 역순 탐색**
   ...
   - Coverage를 4→3→2→1번 역순으로 다시 시작하며, 레인 이동도 waypoint
     뒤 동쪽 벽 ToF로 x를 미세보정한다.
