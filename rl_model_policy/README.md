...
좌우를 뜻하지 않는다.
물리적 왼쪽 센서는 버스 7번, 오른쪽 센서는 버스 1번이며 두 ToF 센서는
같은 벽의 거리와 각도를 함께 계산한다.
...
1. **레인 탐색**
   - 최초 탐색은 1→2→3→4번 레인 순서이며 중심 x는 각각 `1.25`,
     `0.25`, `-0.75`, `-1.25 m`이다.
   - 시작 직후 첫 진입점만 기존 `(1.25, -1.3343) m`를 사용한다. 이 점은
     odometry waypoint로 진입하며 ToF 보정은 수행하지 않는다.
   ...
   - 다음 레인 이동은 도착 레인이 1·2번이면 동쪽 벽, 3·4번이면 서쪽 벽을
     ...
     3 cm 이내로 보정한다. odometry 방향 정렬은 한 번만 수행하고 이후에는
     ToF 각도와 거리만 사용하며, 완료 시 동쪽은 yaw `0도`, 서쪽은 `180도`로
     pose를 보정한다.
...
6. **역순 탐색 재개**
   - 출구 x 보정 뒤 `(-1.25, -1.40) m`로 이동해 북쪽으로 회전한 다음
     역순 탐색을 시작한다.
